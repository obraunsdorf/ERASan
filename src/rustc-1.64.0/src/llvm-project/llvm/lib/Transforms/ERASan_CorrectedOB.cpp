...
static cl::opt<bool> ClErasanOpts(
  "erasan-opts", cl::desc("Enable Erasan optimizations"),
  cl::Hidden, cl::init(true));
...
STATISTIC(AllOperands, "OB: all operands"); 
STATISTIC(InstrumentedOperands, "OB: instrumented operands"); 
...
bool elide_with_erasan(InterestingMemoryOperand &Operand, Function &F) {
  Instruction* OperandSourceInst = dyn_cast<Instruction>(Operand.getPtr());
  Instruction* OperandUseInst = Operand.getInsn();
  Instruction* OperandInstruction;
  // Comment(obraunsdorf): Sometimes Operand.getPtr might not be an instruction but a global, constant or argument
  if (OperandSourceInst) {
    OperandInstruction = OperandSourceInst;
  } else {
    // Comment(obraunsdorf): in this case we can fall back to the instruction that uses the operand, which was the default behavior in ERASan
    // I think this is not correct though.
    errs() << "ERROR: Operand.getPtr is not an instruction: " << *Operand.getPtr() << " in Function " << F.getName() << "\n";
    OperandInstruction = OperandUseInst;
  }
  bool elide;
  if (OperandInstruction->hasMetadata("rawptr")) {
    //errs() << "Instrumenting Operand (rawptr) " << *Operand.getPtr() << " used in MemoryOperation " << *Operand.getInsn() << " of Function " << F.getName() << "\n";
    elide = false;
  } else if (OperandInstruction->hasMetadata("rawptr_deref")) {
    errs() << "Instrumenting Operand (rawptr_deref) " << *Operand.getPtr() << " used in MemoryOperation " << *Operand.getInsn() << " of Function " << F.getName() << "\n";
    elide = false;
  } else if (OperandInstruction->hasMetadata("HeapafterDropAccess")) {
    errs() << "Instrumenting Operand (HeapafterDropAccess) " << *Operand.getPtr() << " used in MemoryOperation " << *Operand.getInsn() << " of Function " << F.getName() << "\n";
    elide = false;
  } else if (OperandInstruction->hasMetadata("StackafterReturnAccess")) {
    errs() << "Instrumenting Operand (StackafterReturnAccess) " << *Operand.getPtr() << " used in MemoryOperation " << *Operand.getInsn() << " of Function " << F.getName() << "\n";
    elide = false;
  } else {
    elide = true;
  }
...
bool AddressSanitizer::instrumentFunction(Function &F,
  ...
  int local_all_operands = 0;
  int local_instrumented_operands = 0;
  /*if (ClInstrumentWrites) {
    errs() << "Yooo OB: ClInstrumentWrites is on\n";
  } else {
    errs() << "Yooo OB: ClInstrumentWrites is off\n";
  }*/
  for (auto &Operand : OperandsToInstrument) {
    local_all_operands++;
    errs() << "Yooo OB: Interesting Operand " << *Operand.getPtr() << " of Instruction " << *Operand.getInsn() << " in Function " << F.getName() << "\n";
    bool elide;
    errs() << "#### ClErasanOpts is " << ClErasanOpts << "\n";
    if (ClErasanOpts) {
      elide = elide_with_erasan(Operand, F);
    } else {
      elide = false;
    }
    if (!suppressInstrumentationSiteForDebug(NumInstrumented)){
        if(!elide){
          errs() << "Operand is instrumented: " << *Operand.getPtr() << " in Function " << F.getName() << "\n";
          instrumentMop(ObjSizeVis, Operand, UseCalls,F.getParent()->getDataLayout());
          local_instrumented_operands++;
        }
    }
  ...
  AllOperands += local_all_operands;
  InstrumentedOperands += local_instrumented_operands;
